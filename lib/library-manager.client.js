...
(function () {
  ...
  function render(target, opts) {
    ...
    function drawRow(row) {
      ...
      del.addEventListener('click', function () { removeRow(row); });

      var rowEl = el('div', { class: 'lmp-row' }, [checkbox, thumb, meta, del]);
    ...
    function loadFromMedia() {
      ...
      return M.getUserMedia().then(function (rows) {
        // Filter to audio/* — the picker does the same (see
        // lib/library-switcher.client.js listLibrary()).
        state.allRows = (rows || [])
          .filter(function (r) { return r && r.blob && r.mime && r.mime.indexOf('audio/') === 0; })
