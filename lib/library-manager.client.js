...
(function () {
  ...
  function render(target, opts) {
    ...
    // ----- cloud-mirror status ----
    // Without this, sync is invisible: a user whose media is missing on
    // another device has no way to tell whether it is still uploading,
    // signed out, or failed. Deliberately a plain text node — the states
    // that matter (offline, error) are the ones a user needs to notice.
    var syncStatus = el('span', { class: 'lmp-sync', text: '' });

    function drawSyncState(st) {
      if (!st || !st.status) { syncStatus.textContent = ''; return; }
      var when = st.lastSyncAt ? new Date(st.lastSyncAt).toLocaleTimeString() : '';
      var text = '';
      if (st.status === 'syncing')     text = 'Syncing…';
      else if (st.status === 'synced') text = 'Backed up' + (when ? ' · ' + when : '');
      else if (st.status === 'offline') text = 'Local only — sign in to back up';
      else if (st.status === 'unavailable') text = 'Local only';
      else if (st.status === 'error')  text = 'Backup failed — your library is safe on this device';
      syncStatus.textContent = text;
      syncStatus.dataset.state = st.status;
      syncStatus.title = st.status === 'error' && st.error ? ('Sync error: ' + st.error) : text;
    }

    var unsubscribeSync = function () {};
    if (window.SWR_MEDIA && typeof window.SWR_MEDIA.onSyncChange === 'function') {
      unsubscribeSync = window.SWR_MEDIA.onSyncChange(drawSyncState);
    }
    if (window.SWR_MEDIA && typeof window.SWR_MEDIA.getSyncState === 'function') {
      drawSyncState(window.SWR_MEDIA.getSyncState());
    }
    ...
    // Deliberately NOT inside .lmp-toolbar: that container is display:none
    // whenever the library is empty, which is exactly when a new user first
    // wonders whether their upload is backed up. This sits in the panel so
    // it is visible in both states.
    var syncBar = el('div', { class: 'lmp-sync-bar' }, [syncStatus]);

    // ----- list + empty state ----
    var list = el('div', { class: 'lmp-list' });
    var empty = el('div', { class: 'lmp-empty', text: 'Your library is empty.' });

    var panel = el('div', { class: 'lmp-panel' }, [tabs, toolbar, syncBar, list, empty]);
    ...
    function recomputeList() {
      ...
      if (!rows.length) {
        ...
        empty.textContent = q
          ...
          : 'Your library is empty. Drop a clip into the media panel on the left to fill it.';
    ...
    // Auto-refresh when the tab regains focus. If the user uploaded a clip
    // via the page's + ADD button while the modal was open in the
    // background, this closes the gap between the manager's last read and
    // the current state of SWR_MEDIA. Cheap to maintain: a single
    // visibilitychange listener per render, removed on destroy.
    function onVisibility() {
      if (document.visibilityState === 'visible') loadFromMedia();
    }
