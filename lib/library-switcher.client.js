...
(function () {
  ...
  async function pick(source, sourceId) {
    ...
    if (source === 'weddings') {
      // Weddings catalog ships metadata + cover art only — no audio
      // bytes. MVM treats pick() returning null here as "preview-only":
      // it adds the song to the playlist (with thumbUrl as the cover)
      // and surfaces a "upload a real audio file to play" hint instead
      // of trying to source bytes that don't ship in v1.
      return null;
    }
    return null;
  }
  ...
  function render(target, opts) {
    ...
    async function selectSource(key) {
      ...
      try {
        ...
        else if (key === 'weddings') songs = await listWeddings();
  ...
  window.SWR_LIBRARY_SWITCHER = {
    ...
    list(source) {
      ...
      if (source === 'weddings') return listWeddings();
