...
(function () {
  ...
  // half-lives). Primitive keys (numbers/strings) are NOT allowed by
  // JS WeakMap, so we wrap them in a fresh Object() each time. The
  // overhead is one allocation per call — the GC reclaims immediately
  // because we hold the only reference.
  var cache = new WeakMap();

  function damp(x, halfLife, instanceId) {
    if (typeof x !== 'number' || isNaN(x)) return 0;
    halfLife = halfLife || 0.4;
    // Always wrap so the WeakMap key is always an object. The user's
    // instanceId (if any) is the canonical key; otherwise we wrap x.
    var key = (instanceId && (typeof instanceId === 'object')) ? instanceId : { v: x };
  ...
  function dampDt(x, halfLife, dt, instanceId) {
    ...
    halfLife = halfLife || 0.4;
    var key = (instanceId && (typeof instanceId === 'object')) ? instanceId : { v: x, t: dt };
