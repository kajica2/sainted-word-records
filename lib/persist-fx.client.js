...
(function () {
  ...
  function snapshotFX() {
    ...
    return {
      ...
      pearl:     s.pearl     || 0,
      glitch:    s.glitch    || 0,
      // Master intensity (0…1) — not part of the persona shape (setPersona
      // ignores unknown keys) but persisted alongside it.
      intensity: (typeof FX.intensity === 'number') ? FX.intensity :
  ...
  function hookFXSetters() {
    ...
    const setters = [
      ...
      'setLiquid', 'setPearl', 'setGlitch', 'setEnabled', 'setIntensity',
    ];
  ...
  function applyFxOnBoot() {
    ...
    // Restore master intensity (its own key wins; persona snapshot is the
    // fallback so old saves without the key keep current behaviour).
    if (persona && typeof persona.intensity === 'number' && isFinite(persona.intensity)) {
      if (typeof FX.setIntensity === 'function') FX.setIntensity(persona.intensity);
      else window.SWR_FX_INTENSITY = Math.max(0, Math.min(1, persona.intensity));
    }
    hookFXSetters();
