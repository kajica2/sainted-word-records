...
(function () {
  ...
  const Library = {
    ...
    async _classify(item) {
      ...
      // Wait for the source to be drawable. The error listener matters as much
      // as the load one: without it a broken image (revoked blob URL, corrupt
      // file) left this promise pending forever AND a later drawImage threw
      // "The HTMLImageElement provided is in the 'broken' state". A timeout is
      // the last line of defence so one bad asset can't stall the pool.
      const ready = await new Promise((res) => {
        if (source.readyState >= 2 || (source.complete && source.naturalWidth > 0)) return res(true);
        const evt = source.tagName === 'VIDEO' ? 'loadeddata' : 'load';
        let done = false;
        const finish = (ok) => { if (done) return; done = true; clearTimeout(timer); res(ok); };
        const timer = setTimeout(() => finish(false), 8000);
        source.addEventListener(evt, () => finish(true), { once: true });
        source.addEventListener('error', () => finish(false), { once: true });
      });
      // Nothing to sample — leave luma/hue at their defaults rather than
      // drawing a broken source.
      if (!ready) return;
      if (item.type === 'video') {
