...
//
// That claim is load-bearing, so the module carries NO Node imports: the one
// function that read versions/*.html off disk (a build-time sanity check)
// lives in scripts/variant-picker-variants.mjs instead. Keeping fs out of
// here is what lets the SAME scoring table run in the browser, so a UI
// auto-pick and the batch CLI cannot drift into two definitions.
...
export const PROFILES = {
  // When BPM is undetectable (vocal-led, dense onsets), fall back to
  // music_video (the gradient preset engine) but only as a floor.
  // Other variants can still beat it if scale/chroma signals are strong.
  music_video: { default: 1.0, bpmBucket_unknown: 0.5 /* reduced from 0.8 — was drowning out all differentiation */ },
  spectrum:    { default: 0.9, bpmBucket_unknown: 0.4 },
  // Every variant now has a fallback weight for BPM-unknown so vocal
  // tracks don't collapse to a single pick. When BPM is known the
  // bpmBucket_* weights dominate.
  film:        { bpmBucket_lo: 1.4, scale_minor: 1.2, energyBucket_lo: 0.6, bpmBucket_unknown: 0.5 },
  neon:        { bpmBucket_hi: 1.5, synthChroma: 1.3, bpmBucket_unknown: 0.5 },
  grid:        { bpmBucket_mid: 1.2, beatStrength: 1.4, bpmBucket_unknown: 0.4 },
  smoke:       { bpmBucket_lo: 1.3, energyBucket_lo: 1.2, bpmBucket_unknown: 0.4 },
  aurora:      { scale_major: 1.5, energyBucket_lo: 1.1, bpmBucket_unknown: 0.6 },
  void:        { energyBucket_lo: 1.4, scale_minor: 1.1, bpmBucket_unknown: 0.5 },
  glitch:      { beatStrength: 1.2, energyBucket_hi: 0.8, chromaVariance: 0.5, bpmBucket_unknown: 0.4 /* was 1.1, reduced — too greedy on hi-energy vocals */ },
  chrome:      { bpmBucket_mid: 1.2, scale_minor: 0.6, chromaVariance: 0.4, bpmBucket_unknown: 0.4 },
  fractal:     { chromaVariance: 1.3, beatStrength: -0.8, bpmBucket_unknown: 0.5 },
  collage:     { bpmBucket_mid: 1.1, beatStrength: 0.8, bpmBucket_unknown: 0.4 },
  pulse:       { energyBucket_hi: 1.4, bpmBucket_lo: 1.0, beatStrength: 0.5, bpmBucket_unknown: 0.3 /* reduced — was firing on every vocal track with dense onsets */ },
  watercolor:  { energyBucket_lo: 1.3, scale_major: 1.0, bpmBucket_unknown: 0.5 },
  eclipse:     { scale_minor: 1.2, energyBucket_mid: 1.0, bpmBucket_unknown: 0.4 },
};
...
const THRESHOLDS = {
  synthChroma: 1.0,
  // chromaVariance threshold tuned against real-world chromagrams:
  // a single-key song has variance ~0.02 (low); a major/minor chord
  // progression ~0.04-0.06; a chromatic / atonal track ~0.10+.
  // fractal should only fire for the truly complex cases, not "vocal
  // with some chord movement" which has variance ~0.05-0.09.
  chromaVariance: 0.10,
  beatStrength: 0.05,
};
...
export function bucketize(analysis) {
  ...
  // hasSignal: we have an audio buffer and the analyzer ran. Distinct
  // from "BPM converged" — onset-based BPM detection fails on vocal
  // / dense-onset material but chroma, onset rate, and scale are still
  // valid. Only beatStrength requires a real BPM.
  const hasSignal = dur > 0 && onsetCount > 0;
  const hasBpm = bpm > 0;
  return {
    bpmBucket: !hasBpm ? 'unknown' : (bpm < 90 ? 'lo' : bpm <= 140 ? 'mid' : 'hi'),
    ...
    // beatStrength requires both a valid BPM AND a reasonable confidence,
    // since multiplying confidence by onsetRate can fake a beat signal
    // out of pure onset density.
    beatStrength: hasBpm ? (a.confidence || 0) * Math.min(onsetRate / 2, 1) : 0,
  };
}
