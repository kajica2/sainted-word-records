...
(function () {
  ...
  function fromStored(s) {
    ...
    if (s.builtIn) {
      const path = (s.id.indexOf('persona/') === 0 || s.id.indexOf('audio/') === 0)
        ? '../library/' + s.id
        : '../library/' + s.id;
      return {
        id: s.id, name: s.name, type,
        tags: s.tags || [],
  ...
  const DEFAULT_REACTOR_PRESETS = [
    ...
    [ { feature: 'treble',   target: 'rot',       scale: 180, ease: 'sharp' },
    ...
    [ { feature: 'bass',     target: 'scale',     scale: 1.5, ease: 'sharp' },
      ...
      { feature: 'mid',      target: 'rot',       scale: 90,  ease: 'sharp' } ],
  ];
  ...
  function hookLib() {
    ...
    if (!Lib.__persist_attached && typeof Lib.addFiles === 'function') {
      ...
      Lib.addFiles = async function (files) {
        await orig(files);
        try { saveLib(); } catch (_) {}
      };
