...
(function () {
  ...
  async function init() {
    ...
    // Phase 10: keyboard shortcuts. Same global keydown listener;
    // ESC remains the only recording-stop shortcut. Space / R / V / Delete
    // are gated on:
    //   - No modifier keys (Ctrl/Cmd/Alt) — so Cmd+R reloads the page
    //     instead of toggling recording
    //   - Focus is NOT on an input/select/textarea/contenteditable — so
    //     typing in the song-modal search box doesn't toggle play
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'v' || e.key === 'V') {
        e.preventDefault();
        // Use the public surface so the DOM checkbox stays in sync.
        if (window.MVM && typeof window.MVM.setVizEnabled === 'function') {
          window.MVM.setVizEnabled(!window.MVM.vizEnabled);
        } else {
          state._vizEnabled = !state._vizEnabled;
        }
        return;
      }
      if (e.key === 'Delete' || e.key === 'Backspace') {
        e.preventDefault();
        const sorted = state.timeline.slice().sort((a, b) => a.startMs - b.startMs);
        const activeRow = sorted.find((t) => state._playheadMs >= t.startMs && state._playheadMs < t.endMs);
        if (activeRow) removeFromTimeline(activeRow.id);
        return;
      }
    });

    // Phase 9: persist the viz-toggle state to localStorage. The
    // user toggles it once, the choice survives a page reload.
    // Default ON if not previously set.
    const VIZ_KEY = 'swr.mvm.viz';
    let storedViz;
    try { storedViz = localStorage.getItem(VIZ_KEY); } catch (_) { storedViz = null; }
    state._vizEnabled = storedViz === null ? true : storedViz !== '0';
    // VIZ toggle — flips state._vizEnabled, which gates the visualizer
    // overlay in renderFrame.
    const vizToggle = $('viz-toggle');
    if (vizToggle) {
      // Reflect the persisted value on the checkbox
      vizToggle.checked = state._vizEnabled;
      vizToggle.addEventListener('change', () => {
        state._vizEnabled = !!vizToggle.checked;
        try { localStorage.setItem(VIZ_KEY, state._vizEnabled ? '1' : '0'); } catch (_) {}
      });
    }
  ...
  // Phase 12 (Weddings handoff): consume swr.pending-playlist +
  // swr.pending-song from localStorage on init. Hydrates SWR_PLAYLIST
  // with the queued rows + (best-effort) restores the pending song.
  // Clears the handoff keys after consumption so refreshes don't
  // re-fire. Returns true if a handoff was consumed.
  async function loadPendingHandoff() {
    let pl, sg;
    try {
      pl = JSON.parse(localStorage.getItem('swr.pending-playlist') || 'null');
      sg = JSON.parse(localStorage.getItem('swr.pending-song')     || 'null');
    } catch (_) { pl = null; sg = null; }
    if (!pl || !Array.isArray(pl) || pl.length === 0) return false;

    // 1) Hydrate SWR_PLAYLIST with the queued rows. SWR_PLAYLIST.add
    //    dedupes by id so this is safe to call multiple times.
    const P = window.SWR_PLAYLIST || (window.SWR && window.SWR.PLAYLIST);
    if (P && typeof P.add === 'function') {
      for (const row of pl) { try { P.add(row); } catch (_) {} }
    }

    // 2) If a pending song is queued, set state.song. tryRestoreSong
    //    will resolve the URL via pick() (returns null for the
    //    weddings source, which is the documented "preview-only"
    //    contract). MVM still surfaces the song name in the header.
    if (sg && sg.source && sg.sourceId) {
      state.song = sg;
      save();
      await tryRestoreSong(state);
    }

    // 3) Clear the handoff so refreshes don't re-fire.
    try {
      localStorage.removeItem('swr.pending-playlist');
      localStorage.removeItem('swr.pending-song');
    } catch (_) {}

    setStatus('opened from weddings: ' + ((sg && sg.title) || (pl[0] && pl[0].title) || pl.length + ' songs'), 'ok');
    return true;
  }
  ...
  window.MVM = {
    ...
    setVizEnabled(enabled) {
      ...
      // Phase 9: persist so the choice survives a reload
      try { localStorage.setItem('swr.mvm.viz', state._vizEnabled ? '1' : '0'); } catch (_) {}
