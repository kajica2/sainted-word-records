...
(function () {
  ...
  async function renderPreview(persona) {
    ...
    try {
      ...
      let result;
      try {
        result = await runPersonaShader(gl, refImg, persona);
      } finally {
        // Free the WebGL context immediately so we don't pile up contexts
        // (Chrome warns at 16+). Without this, every persona preview adds
        // one live context to the GPU until garbage collection catches up.
  ...
  async function buildAll() {
    ...
    for (const [key, persona] of Object.entries(Personas.PERSONAS)) {
      ...
      if (!opt) continue;
      try {
        // Use 2D fallback for thumbnails — they're 48x27 and WebGL would
        // create one context per persona (Chrome warns at 8-16+).
        const dataURL = fallbackThumbnail(persona);
        if (dataURL) {
