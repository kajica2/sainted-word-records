#!/usr/bin/env node
// verify-smoke-automix.mjs — smoke test for Phase 2A port of music_video's
// automix + asset-curator stack into versions/smoke.html.
//
//   node verify-smoke-automix.mjs
//
// Asserts:
//   1. smoke.html loads with no console errors
...
function localServe() {
  return new Promise((resolve) => {
    const server = http.createServer((req, res) => {
      // Rewrite root to versions/smoke.html
      let rel = decodeURIComponent(req.url.split('?')[0].replace(/^\/+/, '')) || 'versions/smoke.html';
      if (rel === 'smoke.html') rel = 'versions/smoke.html';
...
async function run() {
  ...
  try {
    ...
    await page.goto(`http://localhost:${PORT}/versions/smoke.html`, { waitUntil: 'networkidle2', timeout: 30000 });
    ...
    if (realErrors.length === 0) ok('smoke.html loads with no relevant console errors');
