...
- `fx-postprocess.js`, `video-fx.css` — 14-FX WebGL fullscreen-quad pipeline (`window.FX.state`); honours the master `FX.intensity` multiplier (0–1, render-time only — never written into state) exposed by `lib/intensity-slider.client.js` and persisted via `swr.fx.intensity`
...
- `client/`, `lib/` — shared browser client modules (visualizer-controller, auth, media-store, library-manager, library-switcher, migrate, design-tokens, components, nav, capture-runtime, swr-media-input, swr-camera-preview, swr-mic-meter, swr-spit-runtime, swr-spit-fx, media-feat — canonical 12-field audio feat extraction from any media element, `SWR_MEDIA_FEAT.attach(el)`, used by tape for real automix features —, intensity-slider — two-way FX master intensity control)
