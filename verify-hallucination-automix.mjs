#!/usr/bin/env node
// verify-hallucination-automix.mjs — smoke test for Phase 2A port of music_video's
// automix + asset-curator stack into versions/hallucination.html.
//
//   node verify-hallucination-automix.mjs
//
// Asserts:
//   1. hallucination.html loads with no console errors
...
function localServe() {
  return new Promise((resolve) => {
    const server = http.createServer((req, res) => {
      // Rewrite root to versions/hallucination.html
      let rel = decodeURIComponent(req.url.split('?')[0].replace(/^\/+/, '')) || 'versions/hallucination.html';
      if (rel === 'hallucination.html') rel = 'versions/hallucination.html';
...
async function run() {
  ...
  try {
    ...
    await page.goto(`http://localhost:${PORT}/versions/hallucination.html`, { waitUntil: 'networkidle2', timeout: 30000 });
    ...
    if (realErrors.length === 0) ok('hallucination.html loads with no relevant console errors');
