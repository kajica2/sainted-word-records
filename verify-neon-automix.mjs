#!/usr/bin/env node
// verify-neon-automix.mjs — smoke test for Phase 2A port of music_video's
// automix + asset-curator stack into versions/neon.html.
//
//   node verify-neon-automix.mjs
//
// Asserts:
...
function localServe() {
  return new Promise((resolve) => {
    const server = http.createServer((req, res) => {
      // Rewrite root to versions/neon.html
      let rel = decodeURIComponent(req.url.split('?')[0].replace(/^\/+/, '')) || 'versions/neon.html';
      if (rel === 'neon.html') rel = 'versions/neon.html';
...
async function run() {
  ...
  try {
    ...
    await page.goto(`http://localhost:${PORT}/versions/neon.html`, { waitUntil: 'networkidle2', timeout: 30000 });
