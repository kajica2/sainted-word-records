#!/usr/bin/env node
// verify-film-automix.mjs — smoke test for Phase 2A port of music_video's
// automix + asset-curator stack into versions/film.html.
//
//   node verify-film-automix.mjs
//
// Asserts:
//   1. film.html loads with no console errors
...
function localServe() {
  return new Promise((resolve) => {
    const server = http.createServer((req, res) => {
      // Rewrite root to versions/film.html
      let rel = decodeURIComponent(req.url.split('?')[0].replace(/^\/+/, '')) || 'versions/film.html';
      if (rel === 'film.html') rel = 'versions/film.html';
...
async function run() {
  ...
  try {
    ...
    await page.goto(`http://localhost:${PORT}/versions/film.html`, { waitUntil: 'networkidle2', timeout: 30000 });
    ...
    if (realErrors.length === 0) ok('film.html loads with no relevant console errors');
